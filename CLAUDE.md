...
- **Per-card process diagrams (added outside this session, not fully audited)**: ~14 of the cards with a pipeline-style narrative carry an optional 4-step flow shown two ways — `.card-diagram-myslide` (dark diagram, numbered steps with a `.highlight` step) and `.card-flow-chips` (compact chip row with one `.active` chip) — both placed between `.card-desc` and `.card-foot`. A global `.view-mode-switcher` (Dark Diagram / Flow Chips / Text Only) toggles which one shows via `body[data-view-mode="..."]`. Not every card has these (simple utility cards don't); when adding a diagram to a new card, give both blocks matching `data-en` translations like the rest of the card.
- **The view-mode default is tracked in three places that must move together** (same pattern as the language-toggle default): the `.view-mode-btn` that has the static `active` class in the HTML, the JS `var initialMode = '...'` literal, and the JS `localStorage.getItem(VIEW_MODE_KEY) || '...'` fallback. Default was flipped from `diagram` to `chips` 2026-09-16 — all three were changed together; changing only one leaves a mismatch between what the switcher UI shows as active and what's actually rendered on first load.
...
- **Drift goes well beyond new commits — whole features can appear with no heads-up.** On 2026-09-16 this file still said "seven tab panels" with no mention of diagrams/chips/view-mode at all, while the live file already had 8 tabs, a view-mode switcher, and per-card diagrams on ~14 cards — none of it documented here. Don't trust this file's inventory of tabs/cards/features as current; re-grep the actual `index.html` (`data-tab=`, `id="panel-`, card titles, any new global UI blocks near the tabs) before editing, especially after time has passed since this file was last touched.
- **"Remove for now, remember for later" → don't rely on memory alone for the payload.** When the owner asks to pull cards/content out but wants them restorable on request, commit the removal as its own clean commit (not squashed into other changes) and save a memory note pointing at that commit hash — the memory note should say *where to find the exact original markup in git history*, not attempt to re-describe or re-derive the content itself. Restoring later should be a `git show <hash>~1:index.html` lookup, not a rewrite from memory.
